...
type MenuItemButtonProps = Omit<Headless.MenuItemProps<"button">, "as">;
type MenuItemLinkProps = Omit<Headless.MenuItemProps<typeof Link>, "as">;
...
export function DropdownItem({
  ...
  return "to" in props
    ? (
        <Headless.MenuItem as={Link} {...props as MenuItemLinkProps} className={clsx(classes, "cursor-pointer")} />
      )
    : (
        <Headless.MenuItem as="button" type="button" {...props as MenuItemButtonProps} className={clsx(classes, "cursor-pointer")} />
      );
}
  ...
  return (
    ...
    >
      {(Array.isArray(keys) ? keys : keys.split("")).map((char, index) => (
        <kbd
          key={`${char}-${char.charCodeAt(0)}-${Math.random().toString(36).substring(2, 7)}`}
          className={clsx([
